...
def VarianceCalc(relativeSum,   absolute, midpoints, relatives, classes):
    ...
    for i in range(len(absolute)):
        varianceCalc = varianceCalc + \
            ((absolute[i] * (midpoints[i] - average)**2))
...
def empirische_verteilung(classes, absolute, results_empire, midpoints, x_axis):
    ...
    for i in range(len(absolute)):
        # Relative Häufigkeiten Ausrechnen
        re_hau.append(absolute[i] / sum(absolute))
        # iterative addition zur Speicherung
        relativ_add = relativ_add + re_hau[i]
...
def histogramm(classes, absolute, hoe_brei):
    ...
    for i in range(len(absolute)):
        ...
        # Höhe und Breite im 2dimensionalen Array
        hoe_brei.append([k_b[i], kl_hoe[i]])
...
def medianCalc(quantilesX, relativeSum, quantiles, relatives, classes, absolute):
    quantileCalc(quantilesX, relativeSum, quantiles,
                 relatives, classes, absolute)
...
def quantileCalc(quantilesX, relativeSum, quantiles, relatives, classes, absolute):
    ...
    for quantil in quantilesX:
        ...
        for x in range(i+1):
            ...
            r2 = r2 + relatives[x]
            quantilX = classes[i] + \
                ((quantil-r1)/(r2-r1))*(classes[i+1]-classes[i])
...
def deviationCalc(z, quantilesX, relativeSum, quantiles, relatives, midpoints, classes, absolute):  # med; av; NUMBER
    zSum = 0
    if(z == "med"):
        z = medianCalc(quantilesX, relativeSum, quantiles,
                       relatives, classes, absolute)
    elif (z == "av"):
